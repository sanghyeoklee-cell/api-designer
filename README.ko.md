...
웹사이트의 **HTTP API 트래픽을 직접(수동) 녹화**한 뒤, Claude로 **캡처된 엔드포인트를 분석**하고 **바로 실행 가능한 코드 — FastAPI 클라이언트 또는 Claude Agent SDK 에이전트 — 를 생성**하는 로컬 데스크톱 도구입니다. 브라우저는 사용자가 직접 조작하며, 자율 에이전트는 없습니다.
...
3. **생성.** 출력 타깃을 선택해 발견된 API를 그대로 구현하는 실행 가능한 프로젝트를 만듭니다:
   - **FastAPI 클라이언트** — 원본 API를 중계하는 FastAPI 서버/클라이언트.
   - **Claude Agent SDK 에이전트** — 발견된 각 엔드포인트를 커스텀 툴로 래핑한 파이썬 에이전트. 자연어로 Claude를 통해 API를 다룰 수 있습니다.

   비밀값은 환경변수에서 읽으며 코드에 하드코딩하지 않습니다.
...
3. 녹화를 중지하고 세션을 **Analyze**(분석) → 출력 타깃(**FastAPI 클라이언트** 또는 **Claude Agent SDK 에이전트**)을 선택 → **Generate**(코드 생성).
